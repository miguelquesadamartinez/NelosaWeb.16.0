...
//require_once("../../../../gueb/eMiKi/mysql.backup/old/backup.inc");
	echo "<html>\n";
	echo "<head>\n";
   ...
   if (!($link=mysql_connect( "localhost" ,"nelosa","mqm1804")))    { 
	...
	while ($row2 = mysql_fetch_row($result2)) {
		...
		//$tablename_array[$j] = $row2 [0];
		$tablename_array = $row2 [0];
		$j++;
	}
	echo "AA<BR>";
	...
	//echo "SQL: $data_sql_string - ";
	/*while ($j >= 0) {
		echo ("Tabla: " . $row2 [0] . "<br><BR>"); // 
		$tablename_array[$j] = "";
		$tablename_array[$j] = NULL;
		unset($tablename_array[$j]);
		$j--;
	}
	$tablename_array[$j]=NULL;
	unset($tablename_array[$j]);*/
