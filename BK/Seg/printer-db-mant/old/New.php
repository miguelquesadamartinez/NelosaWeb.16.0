...
$link=conectarse(); 
			//	 Comproamos si existe referencia
		 	$result=mysql_query("Select * From " . $impresora . " Where Printer='" . $_Prn . "'",$link);
			$row = mysql_fetch_array($result);
			$bar=(string)$row["Printer"];
			if ($bar=="")
			{
				mysql_query("Insert Into " . $impresora . " (Printer,Ins) VALUES('" . $_Prn . "','" . $_Ins . "')",$link);	
			 	echo "Referencia " . $_Prn . " Insertada";	
			}
			else
			{
